        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data.comment, function (key, value) {
                    ...
                    a += '<div class="commentInfo' + value.idx + '">' + '댓글번호 : ' + value.idx +
                        ' / 작성자 : ' + value.account_idx;
                    if (data.signVO.idx == value.account_idx) {
                        a += '<a onclick="commentUpdate(' + value.idx + ',\'' + value.content +
                            '\');"> 수정 </a>';
                        a += '<a onclick="commentDelete(' + value.idx + ');"> 삭제 </a> </div>';
                    }
                    a += '<div class="commentContent' + value.idx + '"> <p> 내용 : ' + value.content +
                        '</p>';
                    a += '</div></div>';
                    console.log("-------------1");
